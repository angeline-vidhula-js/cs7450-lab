...
function updateBars(house) {
    ...
    let renderedChar = characters.filter(character => {
        if(house !== 'top') {
            if(character.house == house) return character;
        }
        else {
            if(character.power_ranking > 0) return character;
        }
    });
    renderedChar = renderedChar.sort((a, b) => {
        return b.probability_of_survival - a.probability_of_survival;
    });
    renderBars(renderedChar);
}